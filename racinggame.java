// 6% VA OQ 3 wil be eaiser than this
import java.io.*;
import java.util.*;
...
public class racinggame {
    public static void main(String[] args) throws IOException {
        ...
        for (int i = 0; i < N; i++) {
            ...
            if (command == 1) {
                IntegerPair ip = new IntegerPair(value-accum, accum);
                pq.offer(ip);
            } else if (command == 2) {
                accum+=value;
            } else if (command == 3) {
                int position = 1;
                IntegerPair pair = pq.peek();
                ...
                for (int j = 0; j < pairs.size(); j++) {
                    pq.offer(pairs.get(j));
                }
                pw.println(pair.first() + accum);
...
class IntegerPair implements Comparable<IntegerPair> {
    ...
    public int compareTo(IntegerPair o) {
        return this.first() - o.first(); // fastest time only
